...
func TestStaAttemptKillsDHClientBeforeConfWrite(t *testing.T) {
	...
	calls := r.Calls()
	want := []string{"pkill -F " + dhclientPidfile}
...
func TestStaAttemptProceedsWhenDHClientKillFails(t *testing.T) {
	r := NewFakeRunner()
	r.Script("pkill -F "+dhclientPidfile, "", errors.New("exit status 1: no process found"))
	r.Script("wpa_cli -i wlan0 reconfigure", "", nil)
	r.Script("wpa_cli -i wlan0 select_network 0", "", nil)
	r.Script("wpa_cli -i wlan0 status", "wpa_state=COMPLETED\n", nil)
	r.Script("dhclient -v -pf "+dhclientPidfile+" wlan0", "bound to 192.168.3.181\n", nil)
...
// (d) killDHClient itself: issues exactly `pkill -F <dhclientPidfile>` and
// never panics or propagates a failure — it has no return value precisely
// so callers cannot accidentally treat "nothing to kill" as an error.
func TestKillDHClientIssuesPkillAndToleratesFailure(t *testing.T) {
	r := NewFakeRunner()
	r.Script("pkill -F "+dhclientPidfile, "", errors.New("exit status 1"))

	killDHClient(context.Background(), r)

	calls := r.Calls()
	want := "pkill -F " + dhclientPidfile
